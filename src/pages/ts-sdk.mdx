...
Creates a new promo code. A promo code can offer a discount or a free period for a specific set of plans or for all plans in your company.
...
Returns a list of memberships, based on the supplied filters. A membership is an instance of a user purchasing a product - if the membership is valid, the user should have access to the experiences associated with the product. If not, the user will have their product's experiences unfulfilled.
